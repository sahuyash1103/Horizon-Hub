...
router.put("/", authMW, async (req, res) => {
    ...
    if (!groupData.newAdmin) return res.status(400).send("missing field groupData.newAdmin");
    const newAdmin = await User.findOne({ email: groupData.newAdmin });
    if (!newAdmin) return res.status(400).send(`${groupData.newAdmin} not found`);
    ...
    const updatedGroup = await group.updateOne({ $set: { admin: newAdmin._id } }, { new: true });
